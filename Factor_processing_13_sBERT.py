...
# Author: Nicola Drago, Dec. 9th, 2023
...
# New modules to use sBERT and make cluster performance comparisons
...
print(len(l_literature))


# In[2]:


print("PART 1: BAG OF WORDS APPROACH")
...
print(f"The following words were removed from the list because they are so frequent to possibly confuse the clustering:\n{', '.join(top_words)}\n")
...
# Use the suppress_stdout context manager
#with suppress_stdout():
    # Generate embeddings for l_literature
#    embeddings = model.encode(l_literature)
# Nuovo fino a qui
...
# Function to perform PCA and plot scatterplot
#def plot_pca_scatter(data, dimensions, title):
...
# Function to perform PCA and plot scatterplot
def plot_pca_scatter(data, dimensions, title, n_clusters):
    # Perform PCA
    pca = PCA(n_components=dimensions)
    reduced_data = pca.fit_transform(data)
...
# Calculate Silhouette Score for TF-IDF method
#silhouette_score_tfidf = calculate_silhouette_score(tfidf_data, num_clusters)

# Calculate Silhouette Score for sBERT method
silhouette_score_sbert = calculate_silhouette_score(embeddings, num_clusters)

# Print Silhouette Scores
#print(f"Silhouette Score for TF-IDF method: {silhouette_score_tfidf}")
print(f"Silhouette Score for sBERT method: {silhouette_score_sbert}")

# Plot PCA scatterplots
#plot_pca_scatter(tfidf_data, 2, "TF-IDF Cluster Scatterplot")
#plot_pca_scatter(embeddings, 2, "sBERT Cluster Scatterplot")

plot_pca_scatter(embeddings, 2, "sBERT Cluster Scatterplot", num_clusters)
...
# Function to perform PCA and plot 3D scatterplot
def plot_pca_scatter_3d(data, title, n_clusters):
    ...
    # Plot in 3D
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')  # Set the plot to 3D
    scatter = ax.scatter(reduced_data[:, 0], reduced_data[:, 1], reduced_data[:, 2], c=kmeans.labels_)
...
# Plot PCA scatterplots in 3D
plot_pca_scatter_3d(embeddings, "sBERT Cluster Scatterplot 3D", num_clusters)
...
# Function to plot 2D scatterplot using specified PCA components
def plot_pca_scatter_2d(data, pca1, pca2, title, n_clusters):
    # Plot
    plt.figure(figsize=(8, 8))
    plt.scatter(data[:, pca1], data[:, pca2], c=kmeans.labels_)
    plt.xlabel(f'PC{pca1+1}')
    plt.ylabel(f'PC{pca2+1}')
    plt.title(title)
    plt.show()

# Number of clusters
num_clusters = min(len(embeddings), 10)  # Adjust based on your dataset
...
# Perform K-Means clustering
kmeans = KMeans(n_clusters=num_clusters, random_state=42).fit(reduced_data)

# Plot PCA scatterplots
plot_pca_scatter_3d(reduced_data, "sBERT Cluster Scatterplot 3D", num_clusters)
plot_pca_scatter_2d(reduced_data, 0, 1, "PCA1 vs PCA2", num_clusters)
plot_pca_scatter_2d(reduced_data, 1, 2, "PCA2 vs PCA3", num_clusters)
plot_pca_scatter_2d(reduced_data, 2, 0, "PCA3 vs PCA1", num_clusters)
