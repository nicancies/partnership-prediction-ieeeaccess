'''Script to create control corpus for AsDB'''
...
asdb_partnered_projects_folder = f'{corpus_folder}/projects/partner/AsDB'
data_folder = f'{file_path}/data'

evaluation_documents_types = ['pvr','pcr','pper']
...
def analyze_partnered_projects()->dict:
    ...
    if os.path.isdir(asdb_partnered_projects_folder):

        for project in os.listdir(asdb_partnered_projects_folder):
            if os.path.isdir(f'{asdb_partnered_projects_folder}/{project}'):

                # read metadata
                metadata = json.load(open(f'{asdb_partnered_projects_folder}/{project}/metadata.json'))
                ...
                # read documents
                for document in os.listdir(f'{asdb_partnered_projects_folder}/{project}'):
...
def get_control_project(counterpart_project:dict, possible_projects:dict=None)->str:
    '''Get control project for AsDB. It needs to be a project that is not partnered, has at least one evaluation document, and should have closed in the same year as the counterpart project.'''
    ...
    while i < len(possible_projects_keys) and not control_project:
        ...
        if priority_sector and valid:
            if not 'sector' in metadata or metadata['sector'] != counterpart_project['sector']:
                valid = False
...
def create_control_corpus(analyzes: dict):
    '''Create control corpus for AsDB'''
    ...
    for project in analyzes:
        ...
        if control_project_path:
            ...
            print(f'No control project found for project {project}')
