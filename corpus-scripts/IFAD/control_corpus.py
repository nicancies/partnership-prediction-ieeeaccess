'''Script to create control corpus for IFAD'''
...
ifad_partnered_projects_folder = f'{corpus_folder}/projects/partner/IFAD'
data_folder = f'{file_path}/data'

evaluation_documents_types = ['ppa','pcr','pcrd','ppe','mtr','project completion report','project performance evaluation','project performance assessment']
...
def analyze_partnered_projects()->dict:
    ...
    if os.path.isdir(ifad_partnered_projects_folder):

        for project in os.listdir(ifad_partnered_projects_folder):
            if os.path.isdir(f'{ifad_partnered_projects_folder}/{project}'):

                # read metadata
                metadata = json.load(open(f'{ifad_partnered_projects_folder}/{project}/metadata.json'))
                ...
                # read documents
                for document in os.listdir(f'{ifad_partnered_projects_folder}/{project}'):
...
def get_control_project(counterpart_project:dict, possible_projects:dict=None)->str:
    '''Get control project for IFAD. It needs to be a project that is not partnered, has at least one evaluation document, and should have closed in the same year as the counterpart project.'''
...
def create_control_corpus(analyzes: dict):
    '''Create control corpus for IFAD'''
